...
// Watchdog timer configuring
WDTCSR = (1 << WDCE) | (1 << WDE);  // enable wathdog timer configuration mode
WDTCSR = (1 << WDP3) | (1 << WDIE) | (1 << WDP0);  // prescaler, interrupt
...
void loop() {
  ...
  //BOD DISABLE
  //Watchdog timer is set to 8s, which makes single i 8s long
  for(int i = 0; i < 38; i++) {
